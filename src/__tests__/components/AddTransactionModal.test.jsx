...
describe('AddTransactionModal', () => {
  it('renders form fields for adding a transaction', () => {
    render(<AddTransactionModal transaction={null} onSave={vi.fn()} onClose={vi.fn()} />)
  ...
  it('shows "Edit Transaction" title when transaction is provided', () => {
    ...
    render(<AddTransactionModal transaction={txn} onSave={vi.fn()} onClose={vi.fn()} />)
  ...
  it('calls onClose when Cancel is clicked', () => {
    const onClose = vi.fn()
    render(<AddTransactionModal transaction={null} onSave={vi.fn()} onClose={onClose} />)
  ...
  it('calls onSave with form data when valid form is submitted', () => {
    const onSave = vi.fn()
    render(<AddTransactionModal transaction={null} onSave={onSave} onClose={vi.fn()} />)
    ...
    fireEvent.click(screen.getByText('Add Transaction', { selector: 'button' }))
    expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ description: 'Coffee', amount: 5.5 }))
  })
})
